-- Baseline: the schema as Hibernate's ddl-auto had built it when Flyway was adopted.
--
-- Generated from the entity model rather than written by hand, so that it matches an
-- existing database column for column. That matching is the whole point: deployments
-- created before Flyway run with baseline-on-migrate, which marks this version as
-- already applied and starts them at V2. A baseline that merely looked right would
-- leave those databases diverging from new ones with nothing to reveal it.
--
-- Nothing here should be edited. To change the schema, add V2__*.sql beside it.
...
create table content_items (published boolean not null, content_version bigint not null, updated_at timestamp(6) with time zone, content_key varchar(255) not null, content_type varchar(255) not null, id varchar(255) not null, body oid, primary key (id), constraint uk_content_type_key unique (content_type, content_key));
...
create table media_download_jobs (percent float(53) not null, source_height integer, target_height integer, completed_at timestamp(6) with time zone, created_at timestamp(6) with time zone not null, estimated_bytes bigint, expires_at timestamp(6) with time zone, file_size bigint, last_fetched_at timestamp(6) with time zone, started_at timestamp(6) with time zone, mode varchar(32) not null check ((mode in ('PASSTHROUGH','CONVERT'))), state varchar(32) not null check ((state in ('QUEUED','CONVERTING','READY','FAILED','CANCELLED','EXPIRED'))), item_title varchar(512), output_path varchar(1024), source_path varchar(1024), id varchar(255) not null, media_item_id varchar(255) not null, profile_id varchar(255) not null, error oid, primary key (id));
...
create table media_profile_settings (away_max_height integer, download_height integer, show_technical_badges boolean not null, updated_at timestamp(6) with time zone not null, away_behaviour varchar(32) not null check ((away_behaviour in ('ASK','SAVED_ONLY','STREAM'))), id varchar(255) not null, profile_id varchar(255) not null, primary key (id), constraint uk_media_settings_profile unique (profile_id));
create table media_trickplay (columns integer not null, frame_count integer, interval_seconds integer not null, row_count integer not null, sheet_count integer, tile_height integer, tile_width integer, generated_at timestamp(6) with time zone, updated_at timestamp(6) with time zone not null, state varchar(32) not null check ((state in ('GENERATING','READY','FAILED'))), error varchar(512), cache_dir varchar(1024), id varchar(255) not null, media_item_id varchar(255) not null, primary key (id), constraint uk_trickplay_item unique (media_item_id));
...
create table profiles (chess_hints boolean not null, chess_lesson_step integer, chess_wins integer, memory_best integer, narration boolean not null, puzzles_solved integer, stars integer, streak integer, created_at timestamp(6) with time zone, age_mode varchar(255) check ((age_mode in ('YOUNG','OLDER'))), avatar_tint varchar(255), id varchar(255) not null, name varchar(255) not null, owner_id varchar(255) not null, primary key (id));
...
create table watch_party_members (joined_at timestamp(6) with time zone, last_seen_at timestamp(6) with time zone, requested_at timestamp(6) with time zone not null, role varchar(16) not null check ((role in ('HOST','MEMBER','GUEST'))), state varchar(16) not null check ((state in ('PENDING','ADMITTED','DENIED','LEFT'))), display_name varchar(40) not null, guest_token_id varchar(64), poll_token varchar(64), account_id varchar(255), id varchar(255) not null, party_id varchar(255) not null, profile_id varchar(255), primary key (id), constraint uk_party_account unique (party_id, account_id));
